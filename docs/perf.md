...
| 语言 | 时间 (user) | 相对 zona 编译器 |
|------|------------|-----------------|
| C (-O2, double) | 0.027s | 0.17x (基准) |
| Node.js (V8 JIT) | 0.065s | 0.41x |
| **zona 编译器** | **0.16s** | **1x** |
| Ruby | 0.45s | 2.8x |
| Python | 1.05s | 6.6x |
| **zona 解释器** | **1.55s** | **9.7x** |
...
| 测试 | 解释器 | 编译器 | 加速比 |
|------|--------|--------|--------|
| fib(35) | 1.55s | 0.29s | **5.3x** |
| loop(10M) | 0.29s | 0.07s | **4.1x** |

## 性能定位

zona 编译器的性能大致在 **Ruby 与 Node.js 之间**，比 Python 快 3.6 倍，比 C 慢 11 倍。

### 阶段一：虚拟栈骨架

fib(35) 从 0.48s → 0.29s（1.65x 提升），loop 从 0.08s → 0.07s。收益来自直线代码消除 push/pop 函数调用。

### 阶段二：栈原语虚栈化

fib(35) 0.29s → 0.26s。`:dup` `:swap` `:drop` `:over` `:rot` 在虚栈上操作，零运行时开销。提升受限于递归调用开销——阶段三（内联展开）瞄准余下的 push/pop。
